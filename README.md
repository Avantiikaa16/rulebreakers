...
Finishing a region generates a **Tutor Handoff Report** (`/report`) - a reasoning profile built from what the child actually did: which facets they discovered, in what order, in their own words, and what to try next. That's the artefact a real tutor in a Live + AI system would read before a session.
...
- Every AI response is schema-validated (Zod) with a retry-then-fallback path - a malformed or missing model response never breaks the game.
